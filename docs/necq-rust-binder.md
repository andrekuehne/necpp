...
every steer. WP5 owns real mutual-impedance ingestion and a sibling checkout.
...
`dsWeight` is always present; it is `1` when the caller omitted weights.
...
sample = port * samplesPerPort + theta * nPhi + phi
```

This is a transfer envelope of `EmbeddedFarFieldResult`, not a new field kernel.

## Bind once

1. Receive the handoff message (`kind: "isolated-element-characterization"`)
   with the two `ArrayBuffer`s transferred, not cloned.
